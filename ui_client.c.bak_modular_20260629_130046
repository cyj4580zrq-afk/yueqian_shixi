...
static rect_t home_files_btn  = {180, 356, 612, 566};
static rect_t home_sett_btn   = {628, 356, 1000, 566};

/* Voice page (match drawn layout) */
static rect_t voice_mic_btn   = {520, 336, 620, 436};
...
static char g_wsl_ip[64] = "10.222.106.238";
...
static void* send_thread_func(void* arg) {
    ...
    ui_needs_redraw = 1;  /* main loop renders */
    pthread_mutex_unlock(&ui_lock);

    char scp_cmd[512];
    char wsl_ip_snap[64];
    pthread_mutex_lock(&ui_lock);
    strncpy(wsl_ip_snap, g_wsl_ip, sizeof(wsl_ip_snap) - 1);
    wsl_ip_snap[sizeof(wsl_ip_snap)-1] = '\0';
    pthread_mutex_unlock(&ui_lock);
    snprintf(scp_cmd, sizeof(scp_cmd),
             "scp -o StrictHostKeyChecking=no -o ConnectTimeout=10 /root/code/cmd.wav cyj@%s:/home/cyj/workspace/ai_assistant/bin/wav/cmd.wav >/dev/null 2>&1",
             wsl_ip_snap);
...
static void* wifi_scan_thread_func(void* arg) {
    ...
    /* FIX: All blocking I/O (popen+fgets) runs OUTSIDE the lock.
     * Results collected in thread-local buffers, then batch-written under lock. */
    ...
    if (fp) {
        ...
        while (fgets(buf, sizeof(buf), fp) && local_count < MAX_WIFI) {
            ...
            if (strlen(ssid) > 0) {
                strncpy(local_ssid[local_count], ssid, sizeof(local_ssid[0]) - 1);
                local_ssid[local_count][sizeof(local_ssid[0]) - 1] = '\0';
    ...
    /* Only now grab the lock to write shared state */
    pthread_mutex_lock(&ui_lock);
    wifi_count = local_count;
    for (int i = 0; i < local_count; i++) {
        strncpy(wifi_ssid[i], local_ssid[i], sizeof(wifi_ssid[i]) - 1);
        wifi_ssid[i][sizeof(wifi_ssid[i]) - 1] = '\0';
        wifi_signal[i] = local_signal[i];
    }
    wifi_scanning = 0;
    snprintf(status_msg, sizeof(status_msg), "扫描完成，发现 %d 个网络", local_count);
    ui_needs_redraw = 1;  /* main loop will render */
    pthread_mutex_unlock(&ui_lock);
    return NULL;
}
...
static void* weather_thread_func(void* arg) {
    ...
    /* FIX: All blocking I/O (popen+fgets over SSH) runs OUTSIDE the lock.
     * ui_lock is only held for brief shared-state writes + redraws. */
    char local_wuhan[128] = {0};
    char local_beijing[128] = {0};
    int success = 0;
    FILE* fp = popen(cmd, "r");
    if (fp) {
        char buf[256] = {0};
        if (fgets(buf, sizeof(buf), fp)) {
            ...
            if (parts) {
                *parts = '\0';
                ...
                if (wh && bj) {
                    strncpy(local_wuhan, wh + 1, sizeof(local_wuhan) - 1);
                    strncpy(local_beijing, bj + 1, sizeof(local_beijing) - 1);
                    success = 1;
                }
            }
        }
    ...
    /* Only now grab the lock to write shared state + redraw */
    pthread_mutex_lock(&ui_lock);
    if (success) {
        strncpy(weather_info_wuhan, local_wuhan, sizeof(weather_info_wuhan) - 1);
        strncpy(weather_info_beijing, local_beijing, sizeof(weather_info_beijing) - 1);
    ...
    switch (cur_page) {
        case PAGE_HOME:
            if (pt_in_rect(px, py, home_voice_btn)) {
                ...
                ui_needs_redraw = 1;
            } else if (pt_in_rect(px, py, home_files_btn)) {
                cur_page = PAGE_FILES;
        ...
        if (read(touch_fd, &ev, sizeof(ev)) == sizeof(ev)) {
            if (ev.type == EV_ABS) {
                ...
                if (ev.value == 0) {
                    ...
                    handle_touch(px, py);
                    pthread_mutex_unlock(&ui_lock);
                    /* handle_touch() sets ui_needs_redraw when needed.
                     * ONLY the main loop renders to avoid DRM race. */
                }
            }
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    printf("[UI] Ready. Main render loop starting (30fps).\n");

    /* Main render loop: the ONLY thread that calls render_page() + DRMshowUp().
     * Touch/worker threads ONLY set ui_needs_redraw = 1 (under brief lock).
     * This prevents DRM framebuffer races and page-flip deadlocks on RK1808. */
    while (1) {
        usleep(33000);  /* ~30 fps poll */

        /* Check if any thread wants a redraw */
        int need_draw = 0;
        pthread_mutex_lock(&ui_lock);
        if (ui_needs_redraw) {
            ui_needs_redraw = 0;
        ...
        /* msg_timer countdown (1 tick = 33ms) */
        if (msg_timer > 0) {
            msg_timer--;
            if (msg_timer == 0) {
                snprintf(status_msg, sizeof(status_msg), "AI ready");
        ...
        if (need_draw) {
            /* Render with short lock, then flip outside lock */
            pthread_mutex_lock(&ui_lock);
            render_page();
            pthread_mutex_unlock(&ui_lock);
            DRMshowUp(lcd_fb, &DRM);
